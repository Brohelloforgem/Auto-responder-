...
class Bot(commands.AutoShardedBot):
    ...
    async def on_ready(self):
        ...
        print(Fore.RED + r"""
Bot Online 🐕""")
        print(f"{gem}Logged In As {self.user}\nID - {self.user.id}")
        print(f"{gem} Made by gem")
        print(f"{gem}logged In as {self.user.name}")
        print(f"{gem}Total servers ~ {len(self.guilds)}")
        print(f"{gem}Total Users ~ {len(self.users)}")
...
client.owner_ids=gem
gemwizz = os.getenv("token")

async def main():
    await client.start(gemwizz, reconnect=True)
