    ...
    async def select_category(self, interaction: discord.Interaction, select: discord.ui.Select):
        try:
            ...
            if selected_values and "moderation" in selected_values:
                embed = discord.Embed(color=self.color, description="```Kick, Ban, Unban, Mute, Unmute, Lock, Lockall, Unlockall, Addrole, Delrole, Role bots, Clone, Clear, Snipe, Role all, Role, Role humans, Role staus, Role cancel,  Roleremove, Rrole all, Rrole bots, Rrole humans , Rrole status, Rrole cancle, Roleicon, Hideall, Unhideall```")
                ...
                embed = discord.Embed(color=self.color, description="```Play, Stop, Skip, Queue, Clearqueue, Volume, Defaultvolume, Join, Defaultvolume, Move, Leave, Nowplaying, Forward, Rewind, Seek, End,```")
                ...
                embed = discord.Embed(color=self.color, description="```Setup, Setup reqrole, Remove reqrole, Setup config, Setup role, Remove trigger, <trigger> <member>```")
                embed.set_author(name="Trigger Roles Commands")
                await interaction.response.edit_message(embed=embed, view=self)
            elif selected_values and "leaderboard" in selected_values:
                embed = discord.Embed(color=self.color, description="```Leaderboard setup, Leaderboard reset, Leaderboard delete, Leaderboard resend, Leaderboard user, Leaderboard resetuser```")
                embed.set_author(name="Leaderboard Commands")
                await interaction.response.edit_message(embed=embed, view=self)
            elif selected_values and "info" in selected_values:
                embed = discord.Embed(color=self.color, description="```Ping, Uptime, Invite, Botinfo, Supporters, Stats```")
